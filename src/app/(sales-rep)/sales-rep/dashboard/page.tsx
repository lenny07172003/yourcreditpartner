...
export default async function SalesRepDashboardPage() {
  ...
  const { data: rep } = await admin.from("sales_reps").select("id, org_id, first_name, last_name").eq("org_id", orgId).eq("auth_user_id", user.id).eq("status", "active").is("deleted_at", null).maybeSingle();
  if (!rep) redirect("/dashboard");
  const [{ data: rules }, { data: bookings }] = await Promise.all([
    admin.from("calendar_availability").select("weekday, start_time, end_time, timezone, buffer_before_min, buffer_after_min").eq("org_id", rep.org_id).eq("sales_rep_id", rep.id).eq("active", true).order("weekday"),
    admin.from("calendar_bookings").select("id, scheduled_for, status, referrals!inner(client_first_name, client_last_name)").eq("org_id", rep.org_id).eq("sales_rep_id", rep.id).in("status", ["booked", "rescheduled"]).gte("scheduled_for", new Date().toISOString()).order("scheduled_for").limit(10),
  ]);
