...
import { loadBranding } from "@/lib/org/branding";
...
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/public/availability/[token]">) {
  try {
    ...
    const [slots, branding] = await Promise.all([
      getAvailableSlots(admin, salesRepId, orgId),
      loadBranding(admin, orgId),
    ]);
    return NextResponse.json({ referral: { firstName: referral.client_first_name, lastName: referral.client_last_name }, slots, branding });
  } catch (error) {
